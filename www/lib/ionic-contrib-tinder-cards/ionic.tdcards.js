(function(ionic) {
  ...
  var SwipeableCardView = ionic.views.View.inherit({
    ...
    initialize: function(opts) {
      ...
      //Mobile screens have a width / height ratio of roughly 0.6
      //so we're giving more weight to the horizontal swipe areas.
      this.verticalMultiplier = 0.6;
    ...
    isUnderThreshold: function() {
      //return true;
      return Math.abs(this.thresholdAmount) < 0.6 && Math.abs(this.thresholdAmountY) < 0.6;
    },
    /**
    ...
    transitionOut: function(e) {
      ...
      self.onTransitionOut({x: self.thresholdAmount, y: self.thresholdAmountY});
    ...
    _doDrag: function(e) {
      ...
      var self = this;
      setTimeout(function() {
        self.onPartialSwipe({x: self.thresholdAmount, y: self.thresholdAmountY});
      });
    },
    ...
    return {
      ...
      compile: function(element, attr) {
        return function($scope, $element, $attr, swipeCards) {
          ...
          var upText = el.querySelector('.up-text');
          var downText = el.querySelector('.down-text');
          ...
          var swipeableCard = new SwipeableCardView({
            ...
            onPartialSwipe: function(amt) {
              ...
              $timeout(function() {
                ...
                if (self.downText) self.downText.style.opacity = 0;
                if (self.upText) self.upText.style.opacity = 0;
                if(Math.abs(amt.x) > Math.abs(amt.y) * swipeableCard.verticalMultiplier) {
                  if (amt.x < 0) {
                    if (self.leftText) self.leftText.style.opacity = fadeFn(-amt.x);
                  } else {
                    if (self.rightText) self.rightText.style.opacity = fadeFn(amt.x);
                  }
                } else {
                  if (amt.y < 0) {
                    if (self.upText) self.upText.style.opacity = fadeFn(-amt.y);
                  } else {
                    if (self.downText) self.downText.style.opacity = fadeFn(amt.y);
                  }
                }
                $scope.onPartialSwipe(amt);
              });
            },
            ...
            onSnapBack: function(startX, startY, startRotation) {
              ...
              .on('step', function(v) {
                ...
                if (upText) upText.style.opacity = 0;
                if (downText) downText.style.opacity = 0;
              })
              .start();
    ...
    return {
      ...
      controller: ['$scope', '$element', function($scope, $element) {
        ...
        this.partial = function(amt) {
          ...
          secondCard && bringCardUp(secondCard, amt.x, 4);
          thirdCard && bringCardUp(thirdCard, amt.x, 8);
        };
      }]
    }
  }])
